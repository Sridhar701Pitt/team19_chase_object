...
lower= np.array([73,151,73],np.uint8) # Array (H,S,V) for the lower threshold bound of the HSV image
upper= np.array([75,153,75],np.uint8) # Array (H,S,V) for the upper threshold bound of the HSV image
error = np.array([5,7,7],np.uint8) # Array of error widths to create the upper and lower threshold bounds above.
...
def get_image(CompressedImage):
    ...
    for lower_upperHSV in greensBlobArr:
        mask = mask + cv2.inRange(imgHSV, lower_upperHSV[0], lower_upperHSV[1])
...
while not rospy.is_shutdown():
    ...
    if start:
        ...
        if k == 49: #number 1
            ...
            #Decrease the min pixel area of the tracked object #212 works
